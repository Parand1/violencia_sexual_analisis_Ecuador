...
-- SCRIPT: 01_etl_egresosnor_2019.sql
-- DESCRIPCIÓN: Creación y normalización de la tabla de egresos hospitalarios 2019 en BigQuery.
--               Incluye decodificación de variables numéricas del INEC (edad, sector,
--               especialidad médica, sexo, etnia, provincia, área y condición de egreso).
-- OPTIMIZACIÓN: Particionamiento por fecha_egr_dt y clustering por sector_normalizado, cie10_codigo.
-- ======================================================================================
...
SELECT
    -- 1. Estandarización de la Edad a días (decodificación de unidad)
    ...
    -- 3. Normalización del Sector hospitalario
    CASE SAFE_CAST(sector AS INT64)
        WHEN 1 THEN 'Público'
        WHEN 2 THEN 'Privado' -- Privado con fines de lucro
        WHEN 3 THEN 'Privado' -- Privado sin fines de lucro
        ELSE 'Otro'
    END AS sector_normalizado,

    -- 4. Conversión segura de tipos de datos
    ...
    -- 6. Decodificación demográfica y geográfica
...
FROM `airy-runway-450418-q9.warehouse.egresos_2019`
WHERE
