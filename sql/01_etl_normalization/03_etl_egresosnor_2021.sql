...
-- SCRIPT: 03_etl_egresosnor_2021.sql
-- DESCRIPCIÓN: Creación y normalización de la tabla de egresos hospitalarios 2021 en BigQuery.
--               Maneja la particularidad del INEC de 2021 donde el diagnóstico contenía
--               descripciones de texto en lugar de códigos alfanuméricos puros.
-- ======================================================================================
...
SELECT
    ...
    -- 2. Manejo de CIE-10 (en 2021 viene como descripción textual en la fuente original)
    ...
    -- 5. Normalización de especialidad de egreso (eliminación de tildes y diacríticos)
    REGEXP_REPLACE(NORMALIZE(UPPER(esp_egrpa), NFD), r'\p{M}', '') AS espegre,

    -- 6. Columnas demográficas y de residencia
    sexo,
    etnia,
    prov_res,
    con_egrpa,
...
FROM `airy-runway-450418-q9.warehouse.egresos_2021`
WHERE
